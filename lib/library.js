...
const jx = {
	pseudoDialog(html, options={}) {
		let { id, className, returnElement, focus } = {
			...{ id:'tbar-dialog', className: 'tbar-dialog', returnElement: undefined, focus: undefined, setup: undefined},
			...options
		};
		...
		if(className) {
			form.parentElement.classList.add(`ap-${className}`);
			form.classList.add(className);
		}

		form.parentElement.style = undefined;
		...
		return new Promise ((resolve, reject) => {
			...
			if(form.elements['cancel']) form.elements['cancel'].onclick = event => {
				resolve(null);
				mp.destroy();
			};
		});
	...
	pseudoPrompt(label='Test', id='tbar-prompt') {
		let html = `
			<label>${label}: <input name="input" type="text"></label>
			<button name="cancel">Cancel</button><button name="ok">OK</button>
		`;
		return this.pseudoDialog(html, {id: 'tbar-prompt', returnElement: 'input', focus: 'input'})
		.then(response => {
			console.log(response);
		});
	},
	pseudoConfirm(label='Test', id='tbar-confirm') {
		let html = `
			<label>${label}</label>
			<button name="cancel">Cancel</button><button name="ok" value="1">OK</button>
		`;
		return this.pseudoDialog(html, {id: 'tbar-confirm', returnElement: 'ok'})
		.then(response => {
			console.log(response);
		});
	},
	pseudoAlert(label='Test', id='tbar-alert') {
		let html = `
			<label>${label}</label>
			<button name="ok" value="1">OK</button>
		`;
		return this.pseudoDialog(html, {id: 'tbar-alert', returnElement: 'ok'})
		.then(response => {
			console.log(response);
		});
	},
...
console.log([...elements].map(i => i.textContent).join(','))
	...
	addMenu(label, className, menuid, items) {
		...
		function addItem(label, action, menu, tooltip, hidden=false) {
			if(label===undefined) {	//	no label => separator
				...
				let [submenu, submenuitems, subbutton] = jx.addMenu(label, undefined, undefined, menu);
		...
		function addItems(items) {
			items.forEach(({label, action, menu, tooltip, hidden}) => {
				addItem(label, action, menu, tooltip, hidden);
			});
		}
